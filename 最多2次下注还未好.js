...
(function() {
    ...
    let userSettings = {
        ...
        prioritizeLiveMatches: true, // 默认优先下注进行中的赛事
    ...
    function showSettingsPanel() {
        ...
        // 优先下注进行中的赛事
        const prioritizeLiveContainer = document.createElement('div');
        prioritizeLiveContainer.style.cssText = `
            display: flex;
            align-items: center;
            margin-bottom: 10px;
        `;
        
        const prioritizeLiveCheckbox = document.createElement('input');
        prioritizeLiveCheckbox.type = 'checkbox';
        prioritizeLiveCheckbox.id = 'prioritize-live-checkbox';
        prioritizeLiveCheckbox.checked = userSettings.prioritizeLiveMatches;
        prioritizeLiveCheckbox.style.cssText = `
            margin-right: 10px;
            width: 16px;
            height: 16px;
        `;
        prioritizeLiveContainer.appendChild(prioritizeLiveCheckbox);
        
        const prioritizeLiveLabel = document.createElement('label');
        prioritizeLiveLabel.htmlFor = 'prioritize-live-checkbox';
        prioritizeLiveLabel.textContent = '优先下注进行中的赛事';
        prioritizeLiveLabel.style.cssText = `
            font-size: 13px;
            cursor: pointer;
        `;
        prioritizeLiveContainer.appendChild(prioritizeLiveLabel);
        betPreferenceSetting.appendChild(prioritizeLiveContainer);
        ...
        betPreferenceDescription.style.cssText = `
            ...
            font-style: italic;
        `;
        betPreferenceDescription.textContent = '默认下注胜率高的一方，勾选反向下注选项后将下注胜率低的一方。可以设置是否优先下注进行中的赛事或提前下注即将开始的赛事。';
        ...
        saveBtn.onclick = function() {
            ...
            userSettings.prioritizeLiveMatches = document.getElementById('prioritize-live-checkbox').checked;
        ...
        for (const match of collectedData.matches) {
            ...
            if (isLiveMatch) {
                ...
                if (!betUpcomingMatches) {
                    console.log(`  跳过: 即将开始的比赛，但设置不下注即将开始的比赛`);
                    continue;
                }
                console.log(`  处理: 即将开始的比赛`);
            } else if (isNotStartedMatch) {
                console.log(`  处理: 未开始的比赛`);
            } else {
    ...
    async function confirmBet() {
        ...
        if (!confirmButton) {
            ...
            const commonSelectors = [
                'div.btBtn.disabled', 'div.btBtn:not(.disabled)', 
            ...
            for (const selector of commonSelectors) {
                ...
                if (button) {
                    ...
                    console.log(`使用选择器 ${selector} 找到确认按钮`);
                    break;
                }
            }
