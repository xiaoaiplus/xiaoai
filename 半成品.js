...
(function() {
    ...
    function getMatchIdentifier(matchElement) {
        ...
        if (teamElements.length >= 2) {
            return `${teamElements[0].textContent.trim()}_vs_${teamElements[1].textContent.trim()}`;
        }
        
        // 如果无法获取队伍名称，使用比赛内容的哈希值作为标识符
        const matchText = matchElement.textContent.trim();
        return `match_${hashString(matchText)}`;
    }
    ...
    function createControlPanel() {
            ...
            <div class="auto-bet-panel-content">
                ...
                <div class="auto-bet-panel-tab-content" id="matches-tab">
                    <div class="match-list-header">
                        ...
                        <button class="match-list-refresh" id="refresh-matches">刷新列表</button>
                    </div>
        ...
        // 添加刷新比赛列表按钮事件
        const refreshBtn = panel.querySelector('#refresh-matches');
        if (refreshBtn) {
            refreshBtn.addEventListener('click', function() {
                if (typeof updateMatchCards === 'function') {
                    updateMatchCards();
                    updateStatus('已刷新比赛列表');
                } else {
                    console.error('Error: updateMatchCards function is not defined');
                    updateStatus('刷新失败: 函数未定义');
                }
            });
        }
    ...
    async function analyzeBetHistory() {
        ...
        try {
            ...
            for (const record of betRecords) {
                ...
                const matchId = `record_${hashString(matchText)}`;
                
                // 增加该比赛的下注计数
                if (!tempMatchBetCounts[matchId]) {
                    tempMatchBetCounts[matchId] = 1;
                } else {
                    tempMatchBetCounts[matchId]++;
                }
                
                debugLog(`投注记录中的比赛: ${matchText}, ID: ${matchId}, 计数: ${tempMatchBetCounts[matchId]}`);
            }
            ...
            for (const match of currentMatches) {
                ...
                currentMatchMap[matchText] = pageMatchId;
                debugLog(`当前页面比赛: ${matchText}, ID: ${pageMatchId}`);
            }
            
            // 更新全局的matchBetCounts对象，尝试匹配当前页面的比赛
            for (const recordMatchId in tempMatchBetCounts) {
                // 从recordMatchId中提取比赛文本
                const recordIdParts = recordMatchId.split('_');
                recordIdParts.shift(); // 移除'record'前缀
                const recordHash = recordIdParts.join('_');
                
                // 尝试在当前页面找到匹配的比赛
                let matched = false;
                for (const currentMatchText in currentMatchMap) {
                    // 如果比赛文本相似度高，则认为是同一场比赛
                    if (textSimilarity(currentMatchText, recordHash) > 0.6) {
                        const pageMatchId = currentMatchMap[currentMatchText];
                        matchBetCounts[pageMatchId] = tempMatchBetCounts[recordMatchId];
                        debugLog(`匹配成功: 投注记录ID ${recordMatchId} 匹配到页面比赛ID ${pageMatchId}`);
                        matched = true;
                        break;
                    }
                }
    ...
    async function main() {
        ...
        // 添加刷新比赛列表按钮事件
        document.getElementById('match-list-container').addEventListener('click', (e) => {
            // 如果点击的是容器本身而不是卡片，则刷新列表
            if (e.target === document.getElementById('match-list-container')) {
                if (typeof updateMatchCards === 'function') {
                    updateMatchCards();
                    updateStatus('已刷新比赛列表');
                } else {
                    console.error('[自动下注] 错误: updateMatchCards函数未定义');
                    updateStatus('刷新比赛列表失败: 函数未定义');
                }
            }
        });
