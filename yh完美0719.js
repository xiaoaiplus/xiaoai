...
(function() {
    ...
    let userSettings = {
        ...
        prioritizeLiveMatches: true, // 默认优先下注进行中的赛事（包括第一局、第二局等）（此设置始终为true且不可关闭）
        betUpcomingMatches: false, // 默认不下注即将开始的赛事（包括BO几总比赛）
        betAmount: 10, // 默认下注金额
        ...
        betAmount1: 15, // 赔率超过阈值1时的下注金额
        oddsThreshold2: 2.60, // 赔率阈值2
        betAmount2: 10 // 赔率超过阈值2时的下注金额
    };
    ...
    function updateBetRecordsInPopup() {
        ...
        if (betRecords.length === 0) {
            ...
            betRecords.forEach(record => {
                ...
                html += `
                <div class="bet-record-item" data-id="${record.id}" data-team-name="${record.teamName}" data-opponent-name="${record.opponentName}" data-match-type="${record.matchType}" style="cursor: pointer;">
                    <div class="bet-record-team"><span style="font-weight: bold; padding: 2px 5px; border-radius: 3px; background: linear-gradient(to right, #ff0000, #ff7f00, #ffff00, #00ff00, #0000ff, #4b0082, #9400d3); -webkit-background-clip: text; -webkit-text-fill-color: transparent; text-shadow: 0 0 2px rgba(255,255,255,0.5);">${record.teamName}</span> vs ${record.opponentName}</div>
                    ...
                    <div style="margin-top: 8px; display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <div>
                            ...
                            <button class="record-btn lose-btn" data-id="${record.id}" style="background-color: rgba(244, 67, 54, 0.7); color: white; border: none; border-radius: 3px; padding: 2px 5px; margin-right: 5px; cursor: pointer; font-size: 11px;">输</button>
                            <button class="record-btn delete-btn" data-id="${record.id}" style="background-color: rgba(158, 158, 158, 0.7); color: white; border: none; border-radius: 3px; padding: 2px 5px; cursor: pointer; font-size: 11px;">删</button>
                        </div>
                    </div>
                </div>
                `;
        ...
        const deleteButtons = recordsContainer.querySelectorAll('.delete-btn');

        winButtons.forEach(btn => {
            btn.addEventListener('click', function(e) {
                e.stopPropagation(); // 阻止事件冒泡，防止触发记录项的点击事件
        ...
        deleteButtons.forEach(btn => {
            btn.addEventListener('click', function(e) {
                e.stopPropagation(); // 阻止事件冒泡，防止触发记录项的点击事件
                const recordId = parseInt(this.getAttribute('data-id'));
                deleteBetRecord(recordId);
            });
        });

        // 添加投注记录项点击事件 - 显示比赛详情
        const recordItems = recordsContainer.querySelectorAll('.bet-record-item');
        recordItems.forEach(item => {
            item.addEventListener('click', function() {
                const teamName = this.getAttribute('data-team-name');
                const opponentName = this.getAttribute('data-opponent-name');
                const matchType = this.getAttribute('data-match-type');

                // 创建一个模拟的比赛对象
                const recordId = parseInt(this.getAttribute('data-id'));
                // 查找对应的投注记录以获取赔率信息
                const betRecord = betRecords.find(record => record.id === recordId);
                
                // 创建赔率对象
                let oddsObj = undefined;
                if (betRecord) {
                    // 在下注记录中，我们只有下注队伍的赔率
                    // 为了显示目的，我们可以为对手设置一个估计的赔率
                    // 通常，如果一个队伍的赔率是X，对手的赔率大约是(1/X) + 一些利润率
                    const mainTeamOdds = betRecord.odds;
                    // 估算对手赔率 (简单估算，实际情况可能不同)
                    const estimatedOpponentOdds = mainTeamOdds > 2 ? (mainTeamOdds / 2).toFixed(2) : (mainTeamOdds * 1.5).toFixed(2);
                    
                    oddsObj = {
                        // 使用队伍名称作为键
                        [teamName]: mainTeamOdds,
                        [opponentName]: estimatedOpponentOdds
                    };
                }
    ...
    // 删除投注记录
    function deleteBetRecord(recordId) {
        const recordIndex = betRecords.findIndex(record => record.id === recordId);
        if (recordIndex !== -1) {
            const teamName = betRecords[recordIndex].teamName;
            betRecords.splice(recordIndex, 1);
            saveBetRecords();
            updateBetRecordsInPopup();
            showNotification(`已删除 ${teamName} 的投注记录`, 'success');
        }
    }
    ...
    function updateTodayMatchesInPopup() {
        ...
        if (uniqueMatches.length > 0) {
            ...
            uniqueMatches.forEach((match, index) => {
                ...
                // 获取比赛类型/游戏名称
                const gameType = match.type || '未知游戏';
                console.log('比赛类型/游戏名称:', gameType); // 添加调试信息
                ...
                // 根据游戏类型设置对应的图标
                if (gameType.includes('英雄联盟') || gameType.toLowerCase().includes('lol')) {
                    gameIconUrl = 'https://es-a8cdn.silver01.com/CdnBullWebV2/assets/images/default/lol.svg';
                    gameName = '英雄联盟';
                } else if (gameType.includes('DOTA') || gameType.toLowerCase().includes('dota')) {
                    gameIconUrl = 'https://es-a8cdn.silver01.com/CdnBullWebV2/assets/images/default/dota.svg';
                    gameName = 'DOTA 2';
                } else if (gameType.includes('CS') || gameType.toLowerCase().includes('cs2')) {
                    gameIconUrl = 'https://es-a8cdn.silver01.com/CdnBullWebV2/assets/images/default/cs.svg';
                    gameName = 'CS2';
                } else if (gameType.includes('王者荣耀') || gameType.toLowerCase().includes('kog')) {
                    gameIconUrl = 'https://es-a8cdn.silver01.com/CdnBullWebV2/assets/images/default/kog.svg';
                    gameName = '王者荣耀';
                } else if (gameType.includes('Apex') || gameType.toLowerCase().includes('apex')) {
                    gameIconUrl = 'https://es-a8cdn.silver01.com/CdnBullWebV2/assets/images/default/al.svg';
                    gameName = 'Apex 英雄';
                } else if (gameType.includes('铁拳') || gameType.toLowerCase().includes('tk8')) {
                    gameIconUrl = 'https://es-a8cdn.silver01.com/CdnBullWebV2/assets/images/default/tk8.svg';
                    gameName = '铁拳8';
                } else if (gameType.includes('Rennsport') || gameType.toLowerCase().includes('rs')) {
                    gameIconUrl = 'https://es-a8cdn.silver01.com/CdnBullWebV2/assets/images/default/rs.svg';
                    gameName = 'Rennsport';
                } else if (gameType.includes('街头霸王') || gameType.toLowerCase().includes('sf6')) {
                    gameIconUrl = 'https://es-a8cdn.silver01.com/CdnBullWebV2/assets/images/default/sf6.svg';
                    gameName = '街头霸王6';
                } else {
                    // 默认图标
                    gameIconUrl = 'https://es-a8cdn.silver01.com/CdnBullWebV2/assets/images/default/esports.svg';
                    gameName = gameType || '未知游戏'; // 确保有游戏名称
                }

                console.log('处理后的游戏名称:', gameName); // 添加调试信息

                // 构建游戏名称的HTML
                const gameInfoHtml = `<div class="game-info">${gameName}</div>`;
                console.log('游戏名称HTML:', gameInfoHtml); // 添加调试信息

                html += `
                    <div class="match-item ${itemClass}" data-match-id="${match.id}">
                        <div class="match-teams">${statusIndicator}<span style="color: #4CAF50; font-weight: bold; display: inline-block;">${gameName}</span> | ${team1} vs ${team2}</div>
    ...
    function showSettingsPanel() {
        ...
        // 添加自定义滚动条样式
        const settingsPanelScrollStyle = document.createElement('style');
        settingsPanelScrollStyle.textContent = `
        ...
        `;
        document.head.appendChild(settingsPanelScrollStyle);
        ...
        content.className = 'settings-content-scroll';
        ...
        // 添加根据赔率调整下注金额的设置
        const adjustBetAmountContainer = document.createElement('div');
        adjustBetAmountContainer.style.cssText = `
            margin-top: 20px;
        ...
        // 添加标题和开关
        const adjustBetAmountHeader = document.createElement('div');
        adjustBetAmountHeader.style.cssText = `
            display: flex;
            align-items: center;
            margin-bottom: 15px;
        `;

        const adjustBetAmountCheckbox = document.createElement('input');
        adjustBetAmountCheckbox.type = 'checkbox';
        adjustBetAmountCheckbox.id = 'adjust-bet-amount-checkbox';
        adjustBetAmountCheckbox.checked = userSettings.adjustBetAmountByOdds;
        adjustBetAmountCheckbox.style.cssText = `
            margin-right: 10px;
            width: 16px;
            height: 16px;
        `;
        adjustBetAmountHeader.appendChild(adjustBetAmountCheckbox);

        const adjustBetAmountLabel = document.createElement('label');
        adjustBetAmountLabel.htmlFor = 'adjust-bet-amount-checkbox';
        adjustBetAmountLabel.textContent = '根据赔率自动调整下注金额';
        adjustBetAmountLabel.style.cssText = `
            font-size: 13px;
            font-weight: bold;
            cursor: pointer;
        `;
        adjustBetAmountHeader.appendChild(adjustBetAmountLabel);
        adjustBetAmountContainer.appendChild(adjustBetAmountHeader);
        ...
        const betAmount1Label = document.createElement('span');
        betAmount1Label.textContent = '调整注额为:';
        betAmount1Label.style.cssText = `
            width: 80px;
            font-size: 13px;
        `;
        threshold1Container.appendChild(betAmount1Label);

        const betAmount1Input = document.createElement('input');
        betAmount1Input.type = 'number';
        betAmount1Input.id = 'bet-amount1-input';
        betAmount1Input.min = '1';
        betAmount1Input.step = '1';
        betAmount1Input.value = userSettings.betAmount1;
        betAmount1Input.style.cssText = `
            width: 70px;
            padding: 5px 10px;
            border: 1px solid #cccccc;
            border-radius: 4px;
            font-size: 13px;
        `;
        threshold1Container.appendChild(betAmount1Input);
        adjustBetAmountContainer.appendChild(threshold1Container);
        ...
        const betAmount2Label = document.createElement('span');
        betAmount2Label.textContent = '调整注额为:';
        betAmount2Label.style.cssText = `
            width: 80px;
            font-size: 13px;
        `;
        threshold2Container.appendChild(betAmount2Label);

        const betAmount2Input = document.createElement('input');
        betAmount2Input.type = 'number';
        betAmount2Input.id = 'bet-amount2-input';
        betAmount2Input.min = '1';
        betAmount2Input.step = '1';
        betAmount2Input.value = userSettings.betAmount2;
        betAmount2Input.style.cssText = `
            width: 70px;
            padding: 5px 10px;
            border: 1px solid #cccccc;
            border-radius: 4px;
            font-size: 13px;
        `;
        threshold2Container.appendChild(betAmount2Input);
        adjustBetAmountContainer.appendChild(threshold2Container);

        // 添加说明文本
        const adjustBetAmountDescription = document.createElement('div');
        adjustBetAmountDescription.style.cssText = `
            font-size: 11px;
            color: #666666;
            margin-top: 5px;
            font-style: italic;
            padding-left: 25px;
        `;
        adjustBetAmountDescription.textContent = '启用后，系统将根据赔率自动调整下注金额。赔率越高，下注金额越低，以控制风险。';
        adjustBetAmountContainer.appendChild(adjustBetAmountDescription);

        // 根据复选框状态控制输入框的启用/禁用
        function updateAdjustBetAmountInputs() {
            const isEnabled = adjustBetAmountCheckbox.checked;
            threshold1Input.disabled = !isEnabled;
            betAmount1Input.disabled = !isEnabled;
            threshold2Input.disabled = !isEnabled;
            betAmount2Input.disabled = !isEnabled;

            // 更新样式
            const opacity = isEnabled ? '1' : '0.5';
            threshold1Container.style.opacity = opacity;
            threshold2Container.style.opacity = opacity;
        }

        // 初始化输入框状态
        updateAdjustBetAmountInputs();

        // 添加复选框事件监听器
        adjustBetAmountCheckbox.addEventListener('change', updateAdjustBetAmountInputs);

        betAmountSetting.appendChild(adjustBetAmountContainer);
        ...
        // 优先下注进行中的赛事（包括第一局、第二局等） - 默认开启且不可关闭
        ...
        betUpcomingLabel.textContent = '提前下注即将开始的赛事（包括BO几总比赛）';
        ...
        betPreferenceDescription.style.cssText = `
            ...
            font-style: italic;
        `;
        betPreferenceDescription.textContent = '默认下注胜率高的一方，勾选反向下注选项后将只下注胜率低的一方（不再下注胜率高的一方）。系统默认优先下注进行中的赛事（包括第一局、第二局等），可以设置是否提前下注即将开始的赛事（包括BO几总比赛）。';
        ...
        saveBtn.onclick = function() {
            ...
            // 保存下注偏好设置
            const isLowWinRateChecked = document.getElementById('low-win-rate-checkbox').checked;
            userSettings.betLowWinRate = isLowWinRateChecked; // 是否开启反向下注
            userSettings.betHighWinRate = !isLowWinRateChecked; // 当开启反向下注时，自动关闭下注胜率高的一方
            ...
            userSettings.preferBO3Total = userSettings.betUpcomingMatches; // 与提前下注即将开始的赛事共用同一个开关
            userSettings.preferFirstWin = userSettings.prioritizeLiveMatches; // 与优先下注进行中的赛事共用同一个开关

            // 保存自动下注间隔设置
            userSettings.autoBetInterval = parseInt(document.getElementById('auto-bet-interval-input').value) || 30;

            // 保存根据赔率调整下注金额的设置
            userSettings.adjustBetAmountByOdds = document.getElementById('adjust-bet-amount-checkbox').checked;
            userSettings.oddsThreshold1 = parseFloat(document.getElementById('odds-threshold1-input').value) || 2.10;
            userSettings.betAmount1 = parseInt(document.getElementById('bet-amount1-input').value) || 15;
            userSettings.oddsThreshold2 = parseFloat(document.getElementById('odds-threshold2-input').value) || 2.60;
            userSettings.betAmount2 = parseInt(document.getElementById('bet-amount2-input').value) || 10;
    ...
    function loadSettings() {
        try {
            ...
            if (savedSettings) {
                ...
                // 确保preferBO3Total和preferFirstWin与betUpcomingMatches和prioritizeLiveMatches保持一致
                userSettings.preferBO3Total = userSettings.betUpcomingMatches;
                userSettings.preferFirstWin = userSettings.prioritizeLiveMatches;
    ...
    // 关闭所有比赛详情弹窗
    function closeAllMatchDetailPopups() {
        const popups = document.querySelectorAll('.match-detail-popup');
    ...
    function showMatchDetail(match) {
        ...
        // 将当前弹窗ID存储为最新的弹窗
        window.currentMatchDetailPopupId = popupId;
        ...
        if (match.odds) {
            console.log('比赛赔率数据:', match.odds); // 调试信息
            ...
            if (match.teams && match.teams.length > 0) {
                ...
                // 尝试使用team-{队名}格式
                odds1 = match.odds[`team-${team1.name}`] || odds1;
                odds2 = match.odds[`team-${team2.name}`] || odds2;
                
                // 尝试使用teamId格式
                if (team1.id) odds1 = match.odds[team1.id] || odds1;
                if (team2.id) odds2 = match.odds[team2.id] || odds2;
                
                // 尝试使用team1-0, team2-0格式
                odds1 = match.odds['team1-0'] || odds1;
                odds2 = match.odds['team2-0'] || odds2;
                
                // 如果还是未知，尝试从对象中获取值
                if (odds1 === '未知' || odds2 === '未知') {
                    const oddsValues = Object.values(match.odds);
                    if (oddsValues.length > 0 && odds1 === '未知') odds1 = oddsValues[0];
                    if (oddsValues.length > 1 && odds2 === '未知') odds2 = oddsValues[1];
                }
            } else {
                // 尝试从对象中获取第一个和第二个值
                const oddsValues = Object.values(match.odds);
        ...
        console.log('详情弹窗处理后的游戏名称:', gameName); // 添加调试信息

        // 构建游戏名称的HTML
        const gameInfoHtml = `<div class="game-info" style="font-size: 14px; color: #555; margin-bottom: 10px; background-color: rgba(0, 0, 0, 0.03); padding: 5px 10px; border-radius: 5px; width: fit-content;">${gameName}</div>`;

        console.log('详情弹窗游戏信息:', gameType, gameIconUrl, gameName); // 添加调试信息
        ...
        let detailHTML = `
            ...
            <div style="background-color: rgba(240, 240, 240, 0.6); padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                <div style="font-size: 18px; font-weight: bold; margin-bottom: 10px; text-align: center;">
                    ${statusIndicator}
                    <span style="color: #4CAF50; font-weight: bold; display: inline-block; padding: 2px 6px; background-color: rgba(76, 175, 80, 0.1); border-radius: 4px;">${gameName}</span> | ${team1} vs ${team2}
                </div>
                    ...
                    <div id="win-rate-data-${popupId}" style="display: block;">
                        <div class="statsInfo" style="margin-top: 10px; background-color: rgba(240, 240, 240, 0.4); border-radius: 6px; padding: 10px;">
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 5px;">
                                ...
                                <div style="width: 80px; text-align: center; font-size: 12px; color: #888888;">${team1}</div>
                            </div>

                            <div class="statsCategory" style="text-align: center; margin: 8px 0; font-weight: bold; color: #555;">胜率</div>

                            <div style="display: flex; justify-content: space-between; align-items: center;">
                                <div style="width: 80px; text-align: center; font-size: 12px; color: #888888;">${team2}</div>
                                <div style="flex-grow: 1; padding: 0 10px;">
                                    <div class="statsBar" style="height: 10px; background-color: rgba(200, 200, 200, 0.3); border-radius: 5px; overflow: hidden; position: relative;">
    ...
    function extractMatchesFromDOM() {
        ...
        console.log('注意：已更新正则表达式以支持带有特殊字符（如"-"和"."）的队伍名称');
            ...
            teamInfoElements.forEach((element, index) => {
                ...
                if (teamMatch) {
                    ...
                    const tempMatch = {
                        ...
                        odds: {},
                        status: matchStatus,
                        type: matchType.trim(), // 确保比赛类型格式一致
    ...
    function addMatchWithDeduplication(newMatch) {
        ...
        if (existingMatchIndex !== -1) {
            ...
            if (newMatch.type && newMatch.type !== '未知比赛' && newMatch.type !== '未知') {
                // 标准化处理比赛类型，去除前后空格
                existingMatch.type = newMatch.type.trim();
                console.log(`更新比赛类型: ${matchKey} -> ${existingMatch.type}`);
            }
    ...
    async function placeBetOnMatch() {
        ...
        for (const record of betRecords) {
            ...
            if (recordTeamNames === teamNames && recordType === currentMatchType) {
                betCount++;
                console.log(`匹配记录: ${record.teamName} vs ${record.opponentName}, 类型=${recordType}, 当前计数=${betCount}`);
            } else {
                console.log(`不匹配: 队伍比较=${recordTeamNames === teamNames}, 类型比较=${recordType === currentMatchType}`);
        ...
        // 设置下注金额
        try {
            if (!await setBetAmount(userSettings.betAmount, odds)) {
                // 检查是否是因为滚球暂停导致的失败
                if (await closeRollingPausePopup()) {
    ...
    async function selectMatchForBet() {
        ...
        const preferBO3Total = userSettings.preferBO3Total;
        const preferFirstWin = userSettings.preferFirstWin;

        // 调试信息：输出当前设置和收集到的比赛数量
        console.log(`当前设置: 赔率范围=${minOdds}-${maxOdds}, 下注胜率高=${betHighWinRate}, 反向下注=${betLowWinRate}, 优先进行中=${prioritizeLiveMatches}, 下注即将开始=${betUpcomingMatches}, 优先总比赛=${preferBO3Total}, 优先单局=${preferFirstWin}`);
        ...
        for (const match of collectedData.matches) {
            ...
            // 判断是否为总比赛胜利（包括BO3、BO5等）
            const isTotalMatch = matchType.includes('BO') && !matchType.includes('第');

            // 判断是否为单局比赛（第一局、第二局、第三局等）
            const isSingleMatch = matchType.includes('第') && matchType.includes('局');

            // 如果是进行中的比赛，优先下注（包括第一局、第二局等）
            if (isLiveMatch) {
                // 进行中的比赛都接受，不需要额外筛选
                console.log(`  接受: 进行中的比赛(${matchType})`);
            }
            // 这里不需要再次检查betUpcomingMatches，因为前面已经检查过了
            else {
                console.log(`  接受: ${isUpcomingMatch ? '即将开始' : '未开始'}的比赛(${matchType})`);
            }
            ...
            if (betLowWinRate) {
                ...
                if (team1WinRate < team2WinRate && team1Odds >= minOdds && team1Odds <= maxOdds) {
                    ...
                    console.log(`  选择: 队伍1 (${team1Name}), 赔率=${team1Odds}, 胜率=${team1WinRate}%, 比赛类型=${match.type || '未知'} (胜率低)`); 
                    ...
                    console.log(`  选择: 队伍2 (${team2Name}), 赔率=${team2Odds}, 胜率=${team2WinRate}%, 比赛类型=${match.type || '未知'} (胜率低)`);
                } else {
                ...
                if (team1WinRate > team2WinRate && team1Odds >= minOdds && team1Odds <= maxOdds) {
                    ...
                    selectedOdds = team1Odds;
                    opponentName = team2Name;
                    console.log(`  选择: 队伍1 (${team1Name}), 赔率=${team1Odds}, 胜率=${team1WinRate}%, 比赛类型=${match.type || '未知'}`);
                } else if (team2WinRate > team1WinRate && team2Odds >= minOdds && team2Odds <= maxOdds) {
                    selectedTeam = match.teams ? match.teams[1] : { name: team2Name };
                    selectedOdds = team2Odds;
                    opponentName = team1Name;
                    console.log(`  选择: 队伍2 (${team2Name}), 赔率=${team2Odds}, 胜率=${team2WinRate}%, 比赛类型=${match.type || '未知'}`);
                } else {
            ...
            if (!selectedTeam) {
                ...
                // 在反向下注模式下，如果两队胜率相等，则随机选择一队
                if (betLowWinRate) {
                    // 反向下注模式下，如果两队胜率相等，则选择赔率更高的一队（通常赔率高意味着胜率预期低）
                    if (team1Odds > team2Odds && team1Odds >= minOdds && team1Odds <= maxOdds) {
                        selectedTeam = match.teams ? match.teams[0] : { name: team1Name };
                        selectedOdds = team1Odds;
                        opponentName = team2Name;
                        console.log(`  选择: 队伍1 (${team1Name}), 赔率=${team1Odds}, 比赛类型=${match.type || '未知'} (赔率更高)`);
                    } else if (team2Odds > team1Odds && team2Odds >= minOdds && team2Odds <= maxOdds) {
                        selectedTeam = match.teams ? match.teams[1] : { name: team2Name };
                        selectedOdds = team2Odds;
                        opponentName = team1Name;
                        console.log(`  选择: 队伍2 (${team2Name}), 赔率=${team2Odds}, 比赛类型=${match.type || '未知'} (赔率更高)`);
                    ...
                    // 正常模式下，如果两队胜率相等，则选择赔率更低的一队（通常赔率低意味着胜率预期高）
                    if (team1Odds < team2Odds && team1Odds >= minOdds && team1Odds <= maxOdds) {
                        selectedTeam = match.teams ? match.teams[0] : { name: team1Name };
                        selectedOdds = team1Odds;
                        opponentName = team2Name;
                        console.log(`  选择: 队伍1 (${team1Name}), 赔率=${team1Odds}, 比赛类型=${match.type || '未知'} (赔率更低)`);
                    } else if (team2Odds < team1Odds && team2Odds >= minOdds && team2Odds <= maxOdds) {
                        selectedTeam = match.teams ? match.teams[1] : { name: team2Name };
                        selectedOdds = team2Odds;
                        opponentName = team1Name;
                        console.log(`  选择: 队伍2 (${team2Name}), 赔率=${team2Odds}, 比赛类型=${match.type || '未知'} (赔率更低)`);
                    } else if (team1Odds >= minOdds && team1Odds <= maxOdds) {
                        selectedTeam = match.teams ? match.teams[0] : { name: team1Name };
                        selectedOdds = team1Odds;
                        opponentName = team2Name;
            ...
            if (selectedTeam) {
                ...
                eligibleMatches.push({
                    ...
                    status: match.status,
                    matchType: match.type || '未知'
                });
                console.log(`  添加到符合条件的比赛列表: ${selectedTeam.name || '未知队伍'} vs ${opponentName}, 赔率: ${selectedOdds}, 比赛类型: ${match.type || '未知'}`);
            }
        }
        ...
        if (eligibleMatches.length === 0) {
            ...
            console.log(`  优先下注进行中比赛(包括第一局、第二局等): ${prioritizeLiveMatches}`);
            console.log(`  提前下注即将开始比赛(包括BO几总比赛): ${betUpcomingMatches}`);
        ...
        return {
            ...
            matchType: selectedMatch.matchType || '未知'
        };
    }

    // 设置下注金额
    async function setBetAmount(amount, odds) {
        ...
        // 根据赔率调整下注金额
        if (userSettings.adjustBetAmountByOdds && odds) {
            const oddsValue = parseFloat(odds);
            console.log(`检查是否需要根据赔率(${oddsValue})调整下注金额`);

            // 根据赔率阈值调整下注金额
            if (oddsValue >= userSettings.oddsThreshold2) {
                console.log(`赔率(${oddsValue})超过阈值2(${userSettings.oddsThreshold2})，调整下注金额为${userSettings.betAmount2}`);
                amount = userSettings.betAmount2;
            } else if (oddsValue >= userSettings.oddsThreshold1) {
                console.log(`赔率(${oddsValue})超过阈值1(${userSettings.oddsThreshold1})，调整下注金额为${userSettings.betAmount1}`);
                amount = userSettings.betAmount1;
            } else {
                console.log(`赔率(${oddsValue})未超过任何阈值，使用默认下注金额${amount}`);
            }
        }
    ...
    async function confirmBet() {
        ...
        // 检查是否出现投注金额缺失弹窗
        if (await handleMissingAmountPopup()) {
            console.log('确认下注后检测到投注金额缺失弹窗，已处理');
            showNotification('已处理投注金额缺失问题', 'info');
            // 继续执行，不返回false，因为已经处理了问题
        }

        // 处理确认下注后的确认弹框 - 多次尝试，确保能捕获到弹框
        let confirmDialogHandled = false;
        for (let i = 0; i < 3; i++) {
            if (await handleConfirmationDialog()) {
                console.log(`确认下注后检测到确认弹框，已点击确定按钮 (尝试 ${i+1})`);
                showNotification('已自动点击确认弹框中的确定按钮', 'success');
        ...
        if (await closeMarketClosedPopup()) {
            ...
            showNotification('检测到盘口关闭，请稍后重试', 'warning');
            return false;
        }
    ...
    // 检测并处理投注金额缺失弹窗
    async function handleMissingAmountPopup() {
        console.log('检查是否出现投注金额缺失弹窗');
        try {
            // 查找投注金额缺失弹窗
            ...
            if (popupMsg && popupMsg.querySelector('.pMTxt') &&
                ...
                // 查找"是"按钮
                const yesButton = popupMsg.querySelector('div.btn:not(.btnGrey)');
                if (yesButton) {
                    console.log('点击投注金额缺失弹窗的"是"按钮');
                    try {
                        // 尝试使用模拟鼠标事件点击
                        ...
                        yesButton.dispatchEvent(mousedownEvent);

                        const mouseupEvent = new MouseEvent('mouseup', {
                            bubbles: true,
                            cancelable: true
                        });
                        yesButton.dispatchEvent(mouseupEvent);
            ...
            console.error(`处理投注金额缺失弹窗时出错: ${error.message}`);
            return false;
        }
    }

    // 检测并关闭盘口关闭弹窗
    async function closeMarketClosedPopup() {
        console.log('检查是否出现盘口关闭弹窗');
        try {
            // 查找盘口关闭弹窗
            const popupMsg = document.querySelector('div.popMsg.fadeShow');
            if (popupMsg && popupMsg.querySelector('.pMTitle') && popupMsg.querySelector('.pMTitle').textContent.includes('盘口关闭')) {
                console.log('找到盘口关闭弹窗');

                // 查找关闭按钮 - 精确匹配 class="btn btnGrey"
                const closeButton = popupMsg.querySelector('div.btn.btnGrey');
                if (closeButton) {
                    console.log('点击盘口关闭弹窗的关闭按钮');
                    try {
                        // 尝试使用模拟鼠标事件点击
                        const mousedownEvent = new MouseEvent('mousedown', {
                            bubbles: true,
                    ...
                    const anyButton = popupMsg.querySelector('div.btn.btnGrey');
                    if (anyButton) {
                        console.log('未找到明确的关闭按钮，尝试点击灰色按钮');
                        anyButton.click();
                        await new Promise(resolve => setTimeout(resolve, 300));
                        return true;
                    }
            ...
            console.error(`关闭盘口关闭弹窗时出错: ${error.message}`);
            return false;
        }
    }
    ...
    async function handleConfirmationDialog() {
        console.log('检查是否出现确认下注后的确认弹框');
        try {
            // 等待确认弹框出现
            await new Promise(resolve => setTimeout(resolve, 500));

            // 方法1：精确匹配用户提供的确定按钮 - <div class="btn element-picker-highlight">确定</div>
            const confirmButton = document.querySelector('div.btn.element-picker-highlight');
            if (confirmButton) {
                console.log(`找到element-picker-highlight按钮: ${confirmButton.textContent}`);
                if (confirmButton.textContent.trim() === '确定' || confirmButton.textContent.includes('确定')) {
                    console.log('找到确认弹框中的确定按钮，点击确定');
                    try {
                        // 尝试使用模拟鼠标事件点击
                        const mousedownEvent = new MouseEvent('mousedown', {
                            bubbles: true,
                        ...
                        console.error(`模拟点击确定按钮失败: ${clickError.message}，尝试直接点击`);
                        confirmButton.click();
                        await new Promise(resolve => setTimeout(resolve, 300));
                        return true;
                    }
            ...
            // 方法2：查找确认下注弹框后的确定按钮
            // 先查找确认下注弹框
            const betConfirmDialog = document.querySelector('div.btBtn');
            if (betConfirmDialog && betConfirmDialog.textContent.includes('确认下注') && betConfirmDialog.querySelector('div.btExpR')) {
                console.log('找到确认下注弹框:', betConfirmDialog.textContent);

                // 在页面中查找确定按钮
                const confirmBtns = document.querySelectorAll('div.btn');
                console.log(`找到 ${confirmBtns.length} 个btn元素`);

                for (const btn of confirmBtns) {
                    console.log(`检查btn元素: ${btn.textContent}`);
                    if (btn.textContent.trim() === '确定' || btn.textContent.includes('确定')) {
                        console.log('找到确定按钮，点击确定');
                        btn.click();
                        await new Promise(resolve => setTimeout(resolve, 300));
                        return true;
                    }
            ...
            // 方法3：查找任何class为btn的元素
            const btnElements = document.querySelectorAll('div.btn');
            console.log(`找到 ${btnElements.length} 个btn元素`);

            for (const btn of btnElements) {
                console.log(`检查btn元素: ${btn.textContent}`);
                if (btn.textContent.trim() === '确定' || btn.textContent.includes('确定')) {
                    console.log('找到包含"确定"文本的btn元素，点击确定');
                    btn.click();
                    await new Promise(resolve => setTimeout(resolve, 300));
                    return true;
                }
            ...
            // 方法4：查找确认下注弹框中的确定按钮
            // 先查找包含"确认下注"和"预期获利"的弹框
            const confirmDialogs = Array.from(document.querySelectorAll('div.btBtn, div.popMsg, div[class*="dialog"], div[class*="popup"]'));
            const confirmDialog = confirmDialogs.find(dialog =>
                dialog.textContent.includes('确认下注') &&
                dialog.textContent.includes('预期获利')
            );

            if (confirmDialog) {
                console.log('找到确认下注弹框');
                // 在弹框中查找确定按钮
                const confirmBtns = confirmDialog.querySelectorAll('div.btn, button, [role="button"], [class*="btn"]');
                for (const btn of confirmBtns) {
                    if (btn.textContent.trim() === '确定' || btn.textContent.includes('确定')) {
                        console.log('在确认下注弹框中找到确定按钮，点击确定');
                        btn.click();
                        await new Promise(resolve => setTimeout(resolve, 300));
                        return true;
                    }
            ...
            // 方法5：如果没有找到特定的确定按钮，尝试查找任何包含"确定"文本的按钮
            const anyConfirmButtons = Array.from(document.querySelectorAll('div.btn, button, [role="button"], [class*="btn"]'));
            const confirmBtn = anyConfirmButtons.find(btn => btn.textContent.trim() === '确定' || btn.textContent.includes('确定'));
            if (confirmBtn) {
                console.log('找到包含"确定"文本的按钮，点击确定');
                confirmBtn.click();
                await new Promise(resolve => setTimeout(resolve, 300));
                return true;
            }
            ...
            console.error(`处理确认弹框时出错: ${error.message}`);
            return false;
        }
    }
    ...
    function loadBetRecords() {
        try {
            ...
            if (savedRecords) {
                ...
                console.log('没有找到保存的投注记录，初始化为空数组');
                betRecords = [];
            }
        } catch (e) {
    ...
    function saveBetRecords() {
        try {
            ...
            if (betRecords.length > 0) {
                ...
                console.log(`最新记录: ${latestRecord.teamName} vs ${latestRecord.opponentName}, 类型=${latestRecord.matchType || '未知'}, 时间=${new Date(latestRecord.time).toLocaleString()}`);
            }
        } catch (e) {
