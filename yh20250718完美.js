...
(function() {
    ...
    async function selectMatchForBet() {
        ...
        for (const match of collectedData.matches) {
            ...
            // 如果是即将开始的比赛，根据betUpcomingMatches设置决定是否下注（包括BO几总比赛）
            else if ((isUpcomingMatch || isNotStartedMatch) && !betUpcomingMatches) {
                console.log(`  跳过: ${isUpcomingMatch ? '即将开始' : '未开始'}的比赛(${matchType})，因为未开启提前下注`);
                continue;
            }
            else {
